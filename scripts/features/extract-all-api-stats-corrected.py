#!/usr/bin/env python3
"""
Extraction CORRIGEE des stats API Football pour 1760 matches
Objectif: Populer colonnes vides depuis JSON existants
Version corrigee sans colonnes inexistantes
"""
...
def extract_api_stats_from_json(stats_df):
    """Extraire toutes les stats API depuis JSON - VERSION CORRIGEE"""
    print(f"\nEXTRACTING API STATS FROM JSON - CORRECTED VERSION")
    print("=" * 55)
    ...
    # Mapping CORRIGE - seulement les colonnes qui existent vraiment
    api_mappings = {
        ...
        # NOTE: blocked_shots REMOVED - column doesn't exist
    }
    
    print(f"[MAPPING] {len(api_mappings)} valid columns to extract")
    ...
    for idx, record in stats_df.iterrows():
        try:
            ...
            # Extraire chaque stat disponible
    ...
    for json_key, count in extraction_stats.items():
        ...
        print(f"  {column}: {count}/{len(stats_df)} ({pct:.1f}%)")
    
    return extracted_data

def batch_update_match_statistics(supabase, extracted_data):
    """Update match_statistics par batches"""
    print(f"\nBATCH UPDATE MATCH STATISTICS")
    print("=" * 35)
    ...
    batch_size = 50  # Taille batch pour éviter timeouts
        ...
        try:
            ...
            for record in batch_data:
                try:
                    ...
                    print(f"    [WARNING] Failed to update record {record_id}: {e}")
                    continue
    ...
    print(f"[SUCCESS] {successful_updates} records successfully updated")
    return successful_updates > 0

def aggregate_to_team_features_corrected(supabase):
    """Agréger vers team_features - VERSION CORRIGEE"""
    print(f"\nCORRECTED AGGREGATION TO TEAM_FEATURES")
    print("=" * 40)
    ...
    try:
        ...
        # Colonnes à agréger - MISES A JOUR
        aggregation_mapping = {
            ...
            try:
                ...
                if record:  # Si on a des données à update
                    ...
                    if result.data:
                        ...
                        if successful_team_updates % 20 == 0:
        ...
        print(f"[SUCCESS] {successful_team_updates} team features updated")
...
def main():
    """Main corrected extraction process"""
    print("CORRECTED API STATS EXTRACTION - 1760 MATCHES")
    print("=" * 55)
    print("Target: Populer 16 colonnes validees depuis JSON API Football")
    print("Scope: TOUS les match_statistics disponibles")
    print("Fix: Colonne blocked_shots supprimee du mapping")
    ...
    try:
        ...
        print("[OK] Connected for corrected extraction")
        ...
        # Step 2: Extract API stats from JSON
        extracted_data = extract_api_stats_from_json(stats_df)
        ...
        stats_updated = batch_update_match_statistics(supabase, extracted_data)
        ...
        features_updated = aggregate_to_team_features_corrected(supabase)
        ...
        if features_updated:
            print(f"\nSUCCESS COMPLET - SCHEMA CORRIGE!")
            print(f"- 16 colonnes API Football populees")
        ...
        print(f"[ERROR] Corrected extraction failed: {e}")
        return False

if __name__ == "__main__":
    print("ATTENTION: Script corrige pour extraction massive")
    print("Colonnes problematiques supprimees du mapping")
    print("Estimation: 5-10 minutes d'execution")
    
    # Confirmation sécurité
    response = input("\nContinuer avec l'extraction corrigee? (y/N): ")
