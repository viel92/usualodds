#!/usr/bin/env python3
"""
Extraction MASSIVE des stats API Football pour 1760 matches
Objectif: Populer 15 colonnes vides depuis JSON existants
"""
...
def extract_api_stats_from_json(stats_df):
    """Extraire toutes les stats API depuis JSON"""
    print(f"\nEXTRACTING API STATS FROM JSON")
    print("=" * 35)
...
def aggregate_to_team_features_massive(supabase):
    """Agréger vers team_features pour tous les matchs"""
    print(f"\nMASSIVE AGGREGATION TO TEAM_FEATURES")
    print("=" * 40)
    ...
    try:
        ...
        aggregation_mapping = {
            ...
            'expected_goals': 'xg_for_avg',  # Bonus: fix XG aussi !
...
def main():
    """Main massive extraction process"""
    print("MASSIVE API STATS EXTRACTION - 1760 MATCHES")
    print("=" * 55)
    print("Target: Populer 15 colonnes vides depuis JSON API Football")
    print("Scope: TOUS les match_statistics disponibles")
    print("Expected impact: +2-3% ML accuracy")
    ...
    try:
        ...
        print("[OK] Connected for massive extraction")
        ...
        features_updated = aggregate_to_team_features_massive(supabase)
        ...
        if features_updated:
            print(f"\nSUCCESS COMPLET!")
            print(f"- 15 colonnes API Football populees")
        ...
        print(f"[ERROR] Massive extraction failed: {e}")
        return False

if __name__ == "__main__":
    print("ATTENTION: Ce script va traiter TOUS les match_statistics")
    print("Estimation: 5-10 minutes d'execution")
    
    # Confirmation sécurité
    response = input("\nContinuer avec l'extraction massive? (y/N): ")
